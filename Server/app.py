...
# **앞에서 한 말을 되묻는 물음은 판정을 건너뛴다.**
#
# 판정기는 사전지식과 적립만 본다. 최근 세 턴을 붙여도 모자랐다 — 대화 중에 심은
# 사실을 "없다"로 잘못 판정하고, 그러면 지시 한 줄이 붙어 **아는 것도 안 꺼낸다.**
# 20턴 3회차로 재니 기억이 90% 에서 52% 로 떨어졌다. 지어내기를 막으려다 기억을
# 절반 죽인 것이다.
#
#   무슨 요일이라고 했지?          0/3   ->  판정을 끄니 3/3
#   내가 다음 주에 뭐 있다고 했지?   1/3   ->  3/3
#   무슨 색이라고 했더라           2/3   ->  3/3
#
# 실패한 것들이 전부 **인용해서 되묻는 꼴**이었다("~라고 했지", "~다고 했더라").
# 그런 물음의 답은 사전지식이 아니라 대화에 있으므로 판정할 일이 아니다.
#
# **"기억나"는 넣지 않는다.** "내 동생 이름 기억나?" 가 지어내기의 대표 사례이고
# 판정기가 그것을 5/6 에서 0/6 으로 잡았다. 여기 넣으면 그게 되살아난다.
