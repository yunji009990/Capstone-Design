...
다음 꼴로 적어라. 아래는 틀만 보인 것이니 낱말은 대화에서 가져온다.
상대가 다음 주 어느 날에 무슨 일을 한다.
상대의 가족 이름이 무엇이고 나이가 몇이다.
상대가 산 물건이 무슨 색이다.
...
# 본보기는 틀만 보인다. 처음에는 "상대가 새로 산 차는 흰색이다"처럼 구체적으로 썼는데
# 요약에 그대로 들어가 160턴 중 62턴을 오염시켰고, 고양이 색을 묻자 "흰색이야"라고
# 답했다. 글자를 맞춰 걷어내 봤지만 모델이 말을 바꿔 베껴서 18턴이 새고 기억은 오히려
# 떨어졌다. 틀만 보이면 베껴 가도 거짓 사실이 아니라서 해가 없다. 걷어내기는 남겨
# 둔다 — 몇 줄을 걷어냈는지가 곧 지시가 먹히는지의 계측기다.
