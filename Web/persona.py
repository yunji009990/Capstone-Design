...
def build_persona(d: dict) -> str:
    ...
    # **예문은 갈래를 따라간다.** 친구 인물에 "정원이란다"가 붙어 있었다 —
    # 바로 윗줄에서 "-어", "-야"로 끝내라고 해 놓고 어른 말투를 본보기로 준 것이다.
    name = (d.get("user_name") or "").strip()
    L.append('- 상대가 같은 것을 되묻거나 헷갈려도 나무라지 않고 답을 차분히 다시 '
             f'말합니다. {st["renote"].format(name=name)}' if name else
