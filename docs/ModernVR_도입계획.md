...
4. Scene_1: `Player_CRC` 는 비활성만 하고 `Player Spawn` 을 지금 리그 자리에 둔다.
   ...
   회전은 멀미가 난다
5. 바닥·벽·테이블·의자 콜라이더
6. 기존 것이 그대로 도는지 확인 — 음성 대화(`RaonVoiceClient`), 인물 스폰(`PersonaSpawner`),
...
- 잔·사진·유품에 `Grabbable` + `AutoGrab` (손 자세를 비워 두면 자동으로 잡는다). 하나에
  몇 분
- 집어 든 물건을 대화에 잇는다: `Grabbable` 의 잡기 이벤트에서 "지금 ○○를 들고 있다" 를
  서버 세션의 사전지식에 얹는 방식을 검토. **서버 API 가 더 필요하다** — 지금 `/session/start`
  는 시작할 때 한 번만 받는다 (`Server/` 수정, 실서버 `~/server` 에도 반영)
