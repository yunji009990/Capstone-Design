...
기준일: 2026-09-13. 담당 범위·미구현 기능·완료 기준은 [AI 작업 지시서](Tripo_팀원_AI_작업지시서.md)를 따른다.
...
```powershell
python tools/tripo_handoff.py build --out tools/_work/handoff/tripo_handoff_20260913.zip
```

같은 이름을 덮어쓰지 않는다. 재발급 때는 다른 파일명을 지정한다.
묶음은 `.gitignore`와 명시한 소스 경로만 선택하며 Git의 현재 브랜치·index·커밋을 바꾸지 않는다.
현재 삭제된 옛 Raon 스크립트는 삭제 목록과 기준 해시만 전달한다. 삭제된 파일 본문을 ZIP에 넣지 않는다.
...
Unity를 열기 전에 ZIP의 `tripo_handoff.py`로 적용한다. 경로는 자기 PC에 맞춘다.

```powershell
git -C C:/work/Capstone-Design switch -c feat/tripo-studio ba1504a
python C:/handoff/tripo_handoff.py check C:/handoff/tripo_handoff_20260913.zip --repo C:/work/Capstone-Design
python C:/handoff/tripo_handoff.py apply C:/handoff/tripo_handoff_20260913.zip --repo C:/work/Capstone-Design
```
...
```powershell
python -B -m unittest discover -s Survey/tests -p test_model_worker.py -v
python -B -m unittest discover -s tools/tests -p test_tripo_handoff.py -v
```
