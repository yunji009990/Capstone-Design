...
| 웹 등록 흐름을 고친다 | `Web/README.md` — **웹은 2026-09-09 부터 서버에서 돈다** |
...
### 등록 웹도 서버에 있다 (2026-09-09 부터)

**각자 PC 에서 띄우지 않는다.** `http://220.69.208.201:8500` 하나뿐이다.
...
**`~/webapp` 은 `Web/` + `Survey/core/` 의 사본이다.** 고쳤으면 `scp` 로 올리고
`./web_stop.sh; ./web_start.sh`. Raon 과 같은 방식이다.

- **마이크를 안 쓰기 때문에 옮길 수 있었다.** 브라우저 녹음을 넣게 되면 https 가
  필요하다 — 크롬은 localhost 아닌 http 에서 마이크를 막는다
- **화자 분리는 서버에 없다.** 「한 명 (분리 안 함)」으로만 등록된다
- **3D 키를 서버에 상주시키지 않는다.** 공용 기계다. `POST /admin/tripo` 로
  넣고 시연이 끝나면 지운다(유니티 창에도 칸이 있다)
- 유니티의 「서버 켜기」는 없앴다. 이제 「등록 화면 열기」뿐이다
