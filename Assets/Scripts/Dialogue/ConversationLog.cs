...
public class ConversationLog : MonoBehaviour
{
    ...
    void OnEnable()
    {
        if (voice != null)
        {
            voice.OnAnswer += Append;
            voice.OnTranscriptUpdated += UpdateHeard;
            voice.OnAnswerUpdated += UpdateAnswer;
            voice.OnAnswerInterrupted += AppendInterrupted;
        }
    ...
    {
        _pendingHeard = _pendingAnswer = "";
        if (!string.IsNullOrEmpty(heard))
            _text.AppendLine($"<color=#8A93A3>{userName}</color>  {Plain(heard)}");
        if (!string.IsNullOrEmpty(answer))
            _text.AppendLine($"<color=#C58FA0>인물</color>  {Plain(answer)}");
        _text.AppendLine();
    ...
    void UpdateHeard(string heard)
    {
        _pendingHeard = heard;
        Redraw();
    }

    void UpdateAnswer(string heard, string answer)
    {
        _pendingHeard = heard;
    ...
    void AppendInterrupted(string heard, string answer)
        => Append(heard, string.IsNullOrEmpty(answer) ? "" : answer + " (중단)");
    ...
    {
        if (body == null) return;
        var display = new StringBuilder(_text.ToString());
        if (!string.IsNullOrEmpty(_pendingHeard))
            display.AppendLine($"<color=#8A93A3>{userName}</color>  {Plain(_pendingHeard)}");
        if (!string.IsNullOrEmpty(_pendingAnswer))
            display.AppendLine($"<color=#C58FA0>인물</color>  {Plain(_pendingAnswer)}");
        body.text = display.Length == 0 ? "<color=#5A6172>아직 오간 말이 없습니다.</color>"
                                       : display.ToString();
    }
