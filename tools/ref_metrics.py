...
def analyze(path, answer=""):
    ...
    # 말이 느린 것은 쉼과 다른 문제다. 꼬리도 내부 쉼도 정상인데 "말이 길게
    # 늘어진다"는 판정이 나온 파일이 있었고(체감속도 0.167, 나머지 다 정상)
    # 이 판정기는 그것을 "정상"이라 했다.
    #
    # 기준은 청취 9건으로 잡았다. **체감속도가 갈랐고 발화속도로는 안 갈렸다** —
    #   느림  0.167 0.200 0.237 0.238 0.289
    #   괜찮  0.131 0.146 0.148 0.161
    # 0.161 과 0.167 사이라 간격이 좁다. 그래서 0.20 이상만 단정하고 그 아래는
    # 물음표를 붙인다. 표본이 늘면 다시 잡을 것.
    #
    # 문서의 "0.103~0.128 초/글자"를 그대로 쓰면 안 된다. 그건 쉼을 뺀
    # 발화속도이고, 사람이 느리다고 느끼는 것은 쉼까지 포함한 쪽이다.
    slow = ""
    if "체감속도" in res:
        slow = ("느림" if res["체감속도"] >= 0.20 else
                "느림?" if res["체감속도"] >= 0.165 else "")
    grit = ("지지직" if res["지지직"] >= 1.2 else
            "지지직?" if res["지지직"] >= 1.0 else "")
    res["판정"] = "+".join([x for x in (pace, slow, grit) if x]) or "정상"
    return res
