...
HEDGE = re.compile(r"기억\s*(이|은)?\s*(잘\s*)?안\s*나|기억이 가물|생각이\s*안\s*나|"
                   ...
                   # **말투가 바뀌면 여기에 구멍이 생긴다** — 위의 "모른단다" 와 같은 사고다.
                   # 2026-09-04 에 셋을 더 놓치고 있었다. 전부 얼버무린 답인데 지어내기로 셌다.
                   #   "생일이 언제였는지 까먹었어. 알려 줄 수 있어?"
                   #   "영화 제목을 까먹었어. 다시 말해 줄 수 있어?"
                   #   "미안, 몰라. 말해 주면 좋겠다."      <- 「주면」이 요청 목록에 없었다
                   r"까먹|(말|얘기|알려) ?(해 )?주(면|시면)|알려 ?줄 수|(말|얘기)해 ?줄 수")
