...
TURNS = [
    ...
    dict(say="할아버지는 어딨어요?", kind="모르는것",
         ...
         # **「계시지」·「계셨지」만으로는 안 된다.** 「어디 계시지?」는 자문이고
         # 「먼 데 잠깐 가 계시지」는 단정이다. 갈라야 오탐이 안 난다 —
         # 장소가 앞에 붙었거나 단정 어미로 끝난 것만 잡는다.
         fatal=r"돌아가|하늘|먼저 (가셨|갔)|건강히|살아 계|묻히|산소|농사|꿈에\s*(뵈|나)|"
               r"계신단다|계실 거|계셔[.,]|나가셨|나갔단다|"
               r"안 계시|세상 뜨|세상을 떠|계셨는데|계셨단다|계셨어[.,]|"
               r"(먼 데|마당|방|밭|집|밖|옆방|거실|텃밭)[^.?!]{0,8}계시|"
               # 할아버지와 함께한 장면은 무엇이든 지어낸 것이다 - 사전지식에 없다.
               # 「할아버지랑 텃밭에서 오이 따 먹던 생각이 난다」
               r"할아버지(랑|와|하고)",
